...
def main():
    ...
    try:
        ...
        # bot.run()
        print("Diablo-V2 Bot is starting safely...")
...
# Cleaned Emotes & Guns Dictionary
emotes_data = {
    # Evo Guns
    'p90': 909049010,
    'm60': 909051003,
...
def load_emotes_from_json():
    """Load emote IDs from emote.json file with your exact format"""
    emotes_file = "emote.json"
...
📱 Player will see:
"—͞Diabloᅠ wants to join your team"
...
[FFDD00]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[00FF00][b][c]BOT MADE BY DIABLO CODEX
"""
        else:
...
def send_team_ghost(teamcode):
    try:
        url = f"https://rizerxghostbd.onrender.com/x/{teamcode}/DIABLO"
        res = requests.get(url, timeout=10)
...
[FFDD00][b][c]
═══════════════════════════════════════
[00D1FF]TEAM CODE   : {teamcode}
[00D1FF]GHOST NAME  : {ghost_names}

[FF00FF]STATUS      : {status}
[00FF00]MESSAGE     : {message}

[FFDD00]═══════════════════════════════════════
[00FF00][b][c]BOT MADE BY DIABLO CODEX
"""
        else:
            return f"[FF0000]❌ ERROR: HTTP {res.status_code}"

    except Exception as e:
        return f"[FF0000]❌ ERROR: {e}"        
...
def get_youtube_info(channel_name):
    try:
        response_json = requests.get(
            f"https://diablo-yt-info.vercel.app/yt-info/yt?channel={channel_name.lstrip('@')}",
            timeout=15
        ).json()
...
async def send_youtube_info(channel_name, chat_type, uid, chat_id, key, iv):
    ...
    if not response_json:
        await safe_send_message(chat_type, "[B][C][FF0000]Failed to fetch YouTube channel info.", uid, chat_id, key, iv)
        return

    # Stats formatting - using new API field names
    subscribers = xMsGFixinG(str(response_json.get("subscribers", "0")))
    views = xMsGFixinG(str(response_json.get("total_views", "0")))
    # Note: New API doesn't provide videos count, so we show "N/A" or you can remove this line
    videos = "N/A"  # or remove this if not needed
...
[C][B][FF00FF]━━━━━━━━━━━
[C][B][FFFFFF]Channel Name : [FFFF00]{response_json.get('channel_name', 'Unknown')}
[C][B][FFFFFF]Channel ID    : [FFFF00]{response_json.get('channel_id', 'Unknown')}
[C][B][FFFFFF]Handle        : [00BFFF]@{channel_name.lstrip('@')}
...
def get_region_info(uid):
    try:
        url = f"https://diablo-region-api.vercel.app/region?uid={uid}"
        res = requests.get(url, timeout=10)
...
═══════════════════════════════════════
[00D1FF]PLAYER NAME : {player_name}
[00D1FF]PLAYER UID  : {player_uid}
[00D1FF]REGION      : {region}
...
def send_tiktok_info(username):
    ...
    try:
        ...
        return f"""
[B][C][00F2EA]╭[FF0050]─[00F2EA]╮[FFFFFF]╔════════════╗
[C][B][FF0050]│[00F2EA]♪[FF0050]│[FFFFFF]║[00F2EA]TIKTOK INFO[FFFFFF]║
[C][B][00F2EA]╰[FF0050]─[00F2EA]╯[FFFFFF]╚════════════╝
[C][B][FF0050]━━━━━━━━━━━━━━━━━━
[C][B][FFFFFF]Username   : [00F2EA]{username_}
[C][B][FFFFFF]Signature  : [00F2EA]{signature}
[C][B][FFFFFF]Followers  : [00F2EA]{followers}
[C][B][FFFFFF]Following  : [00F2EA]{following}
[C][B][FFFFFF]Likes      : [00F2EA]{likes}
[C][B][FFFFFF]Videos     : [00F2EA]{videos}
[C][B][FFFFFF]Private    : [FF0050]{private_status}
[C][B][FF0050]━━━━━━━━━━━━━━━━━━
"""
...
async def send_emote_menu_background(chat_type, uid, chat_id, key, iv, whisper_writer, online_writer):
    page_size = 5
    emote_items = list(emotes_data.items())
    pages = [emote_items[i:i + page_size] for i in range(0, len(emote_items), page_size)]

    for page in pages:
        try:
            # Each line gets random color
            page_text = "\n".join([
                f"{get_random_color()}{name}: {eid}"
                for name, eid in page
            ])

            success = await safe_send_message(
                chat_type, page_text, uid, chat_id, key, iv, whisper_writer, online_writer
            )
...
async def RejectMSGtaxt(squad_owner,uid, key, iv):
    random_banner = f"""
[000000]
[00FF00]            L O A D I N G . . .
[000000]
[00FFFF]            P L E A S E   W A I T
[000000]
[FFD700]            S Y S T E M   R E A D Y
[000000]
[FF1493]            E N J O Y
[000000]
"""
              ...
              # =================== EMOTE DETECTION ONLY ===================
                ...
                if insquad == True:
                    try:
                        ...
                        message = """[[B][C][9B59B6]╔═══◆ WELCOME TO DIABLO BOT ◆═══╗
[7F8C8D]╟──────────────────────────────╢
[FFFFFF]⚡ Powered By : [BB8FCE]DIABLO
[FFFFFF]🎯 Status     : [2ECC71]ONLINE 24/7
[FFFFFF]📱 Contact    : [1ABC9C]@DIABLO_CODEX_BOT
[7F8C8D]╟──────────────────────────────╢
...
async def TcPChaT(ip, port, AutHToKen, key, iv, LoGinDaTaUncRypTinG, ready_event, region , reconnect_delay=0.5):
    ...
    global normal_aliases, evo_aliases, emote_aliases        # At the VERY TOP of your file, with other globals:
    status_response_cache = {}
...
"""
                        ...
                        if inPuTMsG.strip().startswith('/region '):
                            print('Processing region_info command in any chat type')
                            ...
                            if len(parts) < 2:
                                ...
                                loop = asyncio.get_event_loop()
                                with ThreadPoolExecutor() as executor:
                                    region_result = await loop.run_in_executor(executor, get_region_info, target_uid)
                                await safe_send_message(response.Data.chat_type, f"\n{region_result}\n", uid, chat_id, key, iv)

                        if inPuTMsG.strip().startswith('/ght '):
                            print('Processing ghost command in any chat type')

                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C][FF0000]❌ ERROR! Usage: /check <uid>\nExample: /ghost {teamcode} {name}\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:
                                target_teamcode = parts[1]
                                initial_message = f"[B][C]{get_random_color()}\nFetching the player ban status...\n"
                                await safe_send_message(response.Data.chat_type, initial_message, uid, chat_id, key, iv)
                                ...
                                loop = asyncio.get_event_loop()
                                with ThreadPoolExecutor() as executor:
                                    team_ghost_result = await loop.run_in_executor(executor, send_team_ghost, target_teamcode)
                                await safe_send_message(response.Data.chat_type, f"\n{team_ghost_result}\n", uid, teamcode,chat_id, key, iv)                                
                        ...
                        if inPuTMsG.strip().startswith('/visit'):
                            ...
                            if len(parts) < 2:
                                ...
                                await safe_send_message(response.Data.chat_type, final_visit, uid, chat_id, key, iv)                                              # yt info command handler   
...
#GET ITEM INFORMATION 
        ...
        # Use ThreadPoolExecutor to avoid blocking the async loop
                        ...
                        if inPuTMsG.strip().startswith('/list '):
                            ...
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                error_msg = f"[B][C]{get_random_color()}📜 Sending full emote list...\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
                            else:

                                await send_emote_menu_background(response.Data.chat_type, uid, chat_id, key, iv, whisper_writer, online_writer)
                                
                                await safe_send_message(response.Data.chat_type, error_msg, uid, send_emote_menu_background, chat_id, key, iv)                                
...
"""
                        ...
                        if inPuTMsG == '/espam':
                            if not espam_on:
                                ...
                                success_msg = f"[B][C][00FF00]✅ EMOTE SPAM SUCCESSFULLY STARTED!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                        ...
                        if inPuTMsG == '/estop':
                            ...
                            msg = "[B][C][FF0000]⛔ EMOTE SPAM SUCCESSFULLY STOPPED!"
                            P = await safe_send_message(response.Data.chat_type, msg, uid, chat_id, key, iv)
...
[FFFFFF]⚡ [00E5FF]DIABLO_CODEX_BOT 💎
[AAAAAA]Thank you for your continued support
[00BFFF]╠════════════════════╣
[FFD700]⚡ Owner : [FFFFFF]DIABLO
[00FF99]✨ To purchase a GUILD BOT,
[00FF99]please contact us on Telegram:
[FFFFFF]@DIABLO_CODEX_BOT 💎
[00BFFF]╚════════════════════╝
[CCCCCC]✨ Developed by —͟͞͞ </> [00E5FF]DIABLO CODEX ❄️
"""
                        ...
                        if inPuTMsG.strip().startswith('/room'):
                            ...
                            if len(parts) < 2:
                                ...
                                try:
                                    ...
                                    # Create spam packet
                                    spam_packet = await Room_Spam(target_uid, room_id, " —͞Diabloᅠ", key, iv)
...
#me api
                        ...
                        BLOCKED_NAMES = ["allah", "hasib", "diablo", "codex"]  # Add your actual name
...
[C][B][FF0000]/yt              [F1C40F]Get YouTube info
[C][B][00CEC9]/guild           [55EFC4]Get guild info
[C][B][6C5CE7]/math            [FD79A8]Solve maths
[C][B][FF9F43]/all_event [region]     [1ABC9C]Get multiple region event info
[C][B][E84393]/item [item_code]     [00B894]Get item information
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
...
[C][B][FF7675]/espam            [00CEC9]Automatic random emote spam
[C][B][6C5CE7]/estop            [55EFC4]Stop random emote spam
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]
"""
...
[EB2F06]/gali [name]            [38ADA9]Gali spam message
...
[FF6347]━[32CD32]━[7B68EE]━[FF4500]━[1E90FF]━[ADFF2F]━[FF69B4]━[8A2BE2]━[DC143C]━[FF8C00]━[BA55D3]━[7CFC00]━[FFC0CB]"""
                            ...
                            last ="""[B][C][FF0000]┌[CC0000]─┐ [00F2EA]┌[0FF0FC]─┐ [0088CC]┌[00A8FF]─┐
[C][B][FF4D4D]│ ▶ │ [FE2C55]│ ♪ │ [00C3FF]│ ✈ │
[C][B][B30000]└[FF0000]─┘ [00F2EA]└[0FF0FC]─┘ [0088CC]└[00A8FF]─┘

[C][B][FF0000]YT: DIABLO_CODEX   [00F2EA]TT: DIABLO_CODEX   [0088CC]TG: DIABLO_CODEX_BOT

[C][B][FFB703]TIP: [ADFF2F]For Buy Own Guild Tcp Contact me on telegram:diablo_codex
[C][B][FFB703]TIP: [ADFF2F]/v1,/v2,/v3,/v4,/v5,/spam also target user joined a group for work these commands !
[C][B][FFB703]TIP: [ADFF2F]noob,/title,/sticker work in team chat !
"""
...
async def MaiiiinE():
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 DIABLO_CODEX_BOT BOT - INITIALIZING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 DIABLO_CODEX_BOT BOT - STARTING")
    print("=" * 50)
    ...
    os.system('clear')
    print("🤖 DIABLO_CODEX_BOT BOT - CONNECTING")
    print("=" * 50)
    print("┌────────────────────────────────────┐")
    print("│ ██████████████████████████████████ │")
    print("└────────────────────────────────────┘")
    ...
    os.system('clear')
    print("=" * 50)
    print("🤖 DIABLO_CODEX_BOT BOT - ONLINE")
    print("=" * 50)
    print(f"🔹 UID: {TarGeT}")
