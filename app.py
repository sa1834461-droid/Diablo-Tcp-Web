...
def control_bot():
    ...
    act = data.get('action')
    name = data.get('name')
    uid = data.get('uid') # Captured from user input
    pw = data.get('pw')   # Captured from user input
    
    if act == 'start':
        # --- NEW: Write user input to bot.txt before starting ---
        if uid and pw:
            try:
                with open('bot.txt', 'w', encoding='utf-8') as f:
                    f.write(f"uid={uid}\npassword={pw}")
            except Exception as e:
                return jsonify({"status": "error", "message": f"File Write Error: {str(e)}"})

        # Now start the bot as usual
        ...
        return jsonify({"status": "success", "message": "BOT UPDATED & STARTED", "running": True, "rem_sec": -1})
    
    elif act == 'stop' and name in user_sessions:
        user_sessions[name]['proc'].terminate()
        del user_sessions[name]
        return jsonify({"status": "success", "message": "CORE TERMINATED", "running": False})
